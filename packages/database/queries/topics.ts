...
import { message, topic, usersInProjects, usersInTopics } from "../schema";
...
export const getTopic = async (topicId: number, userId: string) => {
  ...
  const result = await db
    ...
    .where(
      and(eq(usersInTopics.userId, userId), eq(usersInTopics.topicId, topicId))
    )
    .limit(1);
...
export const updateTopic = async (
  ...
  const isAdmin = await db
    ...
    .from(usersInTopics)
    .where(
      and(eq(usersInTopics.topicId, topicId), eq(usersInTopics.userId, userId))
    );
